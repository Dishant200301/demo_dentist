...
const PricingAppointmentBlock = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Basic validation
    if (!formState.firstName || !formState.phone || !formState.date || !formState.time) {
      return;
    }
    ...
    try {
      await publicBookingService.createBookingRequest({
        PatientName: `${formState.firstName} ${formState.lastName}`.trim(),
        PatientPhone: formState.phone,
        PatientEmail: formState.email,
        date: formState.date,
        arrivingTime: formState.time,
        notes: `Service: ${formState.service || 'Default'}`
      });
                ...
                >
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-[20px]">
                    ...
                    <div className="flex flex-col">
                      <input
                        ...
                        className={`w-full h-[55px] px-[20px] rounded-[8px] border border-[#E5E7EB] text-[#424C6F] font-medium text-[15px] outline-none focus:border-[#00ADEE] transition-colors`}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-[20px]">
                    <div className="flex flex-col">
                      <div className="relative">
                        <select
                          className={`w-full h-[55px] px-[20px] rounded-[8px] border border-[#E5E7EB] text-[#424C6F] font-medium text-[15px] outline-none appearance-none focus:border-[#00ADEE] transition-colors bg-white`}
                    ...
                    <div className="grid grid-cols-2 gap-2">
                       <input
                          type="date"
                          className={`w-full h-[55px] px-2 rounded-[8px] border ${hasSubmitted && !formState.date ? 'border-[#FA6E6E]' : 'border-[#E5E7EB]'} text-[#424C6F] font-medium text-[12px] outline-none focus:border-[#00ADEE] transition-colors bg-white`}
                          value={formState.date}
                          onChange={(e) => setFormState({ ...formState, date: e.target.value })}
                        />
                        <input
                          type="time"
                          className={`w-full h-[55px] px-2 rounded-[8px] border ${hasSubmitted && !formState.time ? 'border-[#FA6E6E]' : 'border-[#E5E7EB]'} text-[#424C6F] font-medium text-[12px] outline-none focus:border-[#00ADEE] transition-colors bg-white`}
                  ...
                  <div className="pt-[5px]">
                    <button 
                      type="submit" 
                      disabled={isLoading}
                      className="w-full bg-[#00ADEE] text-white font-bold text-[16px] py-[18px] rounded-[8px] hover:bg-[#15224D] transition-colors text-center inline-flex items-center justify-center gap-2"
                    >
                  ...
                  {(hasSubmitted && !isSuccess && (Object.values(formState).some(v => v === '') || errorMessage)) && (
                    <div className="w-full border border-[#FA6E6E] rounded-[30px] sm:rounded-full py-[12px] px-[20px] text-center mt-[20px]">
                      <span className="text-[#FA6E6E] text-[14.5px] font-normal leading-tight">
                  ...
                  {isSuccess && (
                    <div className="w-full border border-[#4CAF50] rounded-full py-[12px] px-[20px] mt-[20px] text-center">
                      <span className="text-[#4CAF50] text-[15px] font-normal leading-tight">Thank you! Your appointment request has been sent.</span>
                    </div>
                  )}
                </form>
              </div>
